/* ------------------------------------------------------------------
   web3.js – multi-wallet support + Monad Testnet + move functions
-------------------------------------------------------------------*/
...
async function connectWallet() {
  ...
  try {
    ...
    setStat(`🔗 Wallet: ${shorten(currentAcc)}`);
    console.log("✅ Connected with", currentAcc);
  } catch (err) {
...
async function sendMove(dir) {
  ...
  try {
    setStat(`⏳ Confirm ${dir} in wallet…`);
    const tx = await fn();
    await tx.wait();
    setStat(`✅ Move ${dir} confirmed`);
  } catch (err) {
    console.error(err);
    alert(err.reason || err.message || "Transaction failed");
    setStat("❌ Transaction failed");
  }
}
...
function attachProviderEvents(p) {
  ...
  p.on("accountsChanged", (accs) => {
    ...
    setStat(currentAcc ? `🔗 Wallet: ${shorten(currentAcc)}` : "🔌 Not connected");
  });
  p.on("chainChanged", (cId) => {
    if (cId !== MONAD.chainId) setStat("⚠️ Wrong network – switch to Monad");
    else setStat(`🔗 Wallet: ${shorten(currentAcc || "")}`);
  });
}
